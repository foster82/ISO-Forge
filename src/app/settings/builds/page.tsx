...
import { ArrowLeft, Database, Trash2, Clock, CheckCircle, XCircle, HardDrive, Filter } from 'lucide-react'
import Link from 'next/link'
import DeleteButton from '@/components/DeleteButton'
import BareMetalDeployButton from '@/components/BareMetalDeployButton'
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="flex items-center justify-between max-w-6xl mx-auto w-full">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-slate-900">Build Outputs Manager</h1>
          </div>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Active Builds</p>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2">
          ...
          <Link 
            href="/settings/builds?status=BUILDING"
            className={clsx(
              "px-4 py-1.5 rounded-full text-xs font-bold transition-all border",
              status === 'BUILDING' ? "bg-amber-600 text-white border-amber-600" : "bg-white text-slate-600 border-slate-200 hover:border-slate-300"
            )}
          >
            Building
          </Link>
                ...
                jobsWithSizes.map((job) => (
                  <tr key={job.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-bold text-slate-900">{job.profile.name}</p>
                        <p className="text-[10px] font-mono text-slate-400">{job.id}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {job.status === 'BUILDING' && <Database className="w-4 h-4 text-amber-500 animate-pulse" />}
                        <span className={clsx(
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end items-center gap-2">
                        ...
                        <DeleteButton 
                          ...
                          id={job.id}
                          confirmMessage="Delete this build record and the generated image file?"
                          iconSize={4}
                        />
                      </div>
                    </td>
